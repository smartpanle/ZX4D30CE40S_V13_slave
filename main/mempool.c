		...
		size_t pre_allocated_mem_size, size_t num_blocks, size_t block_size)
{
	...
	struct hosted_os_mempool *pool = NULL;
	uint8_t *heap = NULL;
	...
	pool = (struct hosted_os_mempool *)CALLOC(1, sizeof(struct hosted_os_mempool));
	...
	if (hosted_os_mempool_init(pool, num_blocks, block_size, heap, str)) {
		ESP_LOGE(TAG, "hosted_os_mempool_init failed\n");
		goto free_buffs;
	}
...
void hosted_mempool_destroy(struct hosted_mempool *mempool)
{
	...
	hosted_os_mempool_unregister(mempool->pool);
	FREE(mempool->pool);
		...
		size_t nbytes, uint8_t need_memset)
{
	...
	mem = hosted_os_memblock_get(mempool->pool);
#else
...
int hosted_mempool_free(struct hosted_mempool *mempool, void *mem)
{
	...
	return hosted_os_memblock_put(mempool->pool, mem);
#else
	FREE(mem);
	return 0;
#endif
