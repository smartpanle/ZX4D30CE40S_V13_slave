...
## ZX4D30CE08S-V13 (ESP32-P4 host + ESP32-C61 slave)

The P4 host enables `CONFIG_ESP_HOSTED_SDIO_CHECKSUM=y`. This slave must
enable `CONFIG_ESP_SDIO_CHECKSUM=y` as well; otherwise its startup packets
have a zero checksum and the host drops them, then times out connecting.
The C61 defaults in this directory enable SDIO, streaming mode, and checksum.
The existing `sdkconfig` also enables checksum because defaults do not
override an already configured value.

Build and flash from this directory using ESP-IDF 6.0.1:

```sh
idf.py build
idf.py -p <C61_SERIAL_PORT> flash monitor
```

For a fresh configuration, run `idf.py set-target esp32c61` before building.
After flashing the C61, restart the P4 host to verify the SDIO handshake.
