...
#ifndef HOSTED_MEMPOOL_LL_H_
#define HOSTED_MEMPOOL_LL_H_
 ...
 * caller.
 */
struct hosted_os_memblock {
    SLIST_ENTRY(hosted_os_memblock) mb_next;
};
...
/* XXX: Change how I coded the SLIST_HEAD here. It should be named:
   SLIST_HEAD(,hosted_os_memblock) mp_head; */
...
struct hosted_os_mempool {
    ...
    STAILQ_ENTRY(hosted_os_mempool) mp_list;
    SLIST_HEAD(,hosted_os_memblock);
 ...
 * hosted_os_memblock_put() function calls this callback instead of freeing the block
 * itself.  Therefore, it is the callback's responsibility to free the block
 * via a call to hosted_os_memblock_put_from_cb().
 *
 ...
 */
typedef os_error_t hosted_os_mempool_put_fn(struct hosted_os_mempool_ext *ome, void *data,
                                     void *arg);

struct hosted_os_mempool_ext {
    struct hosted_os_mempool mpe_mp;

    /* Callback that is executed immediately when a block is freed. */
    hosted_os_mempool_put_fn *mpe_put_cb;
    void *mpe_put_arg;
};
 ...
 */
struct hosted_os_mempool *hosted_os_mempool_info_get_next(struct hosted_os_mempool *,
        struct hosted_os_mempool_info *);
 ...
 * @return os_error_t
 */
os_error_t hosted_os_mempool_init(struct hosted_os_mempool *mp, uint16_t blocks,
 ...
 * @return os_error_t
 */
os_error_t hosted_os_mempool_ext_init(struct hosted_os_mempool_ext *mpe, uint16_t blocks,
                               uint32_t block_size, void *membuf, const char *name);
 ...
 */
os_error_t hosted_os_mempool_clear(struct hosted_os_mempool *mp);
 ...
 * @return                      0 on success, error code on failure.
 */
void hosted_os_mempool_unregister(struct hosted_os_mempool *mp);
 ...
 * @return os_error_t
 */
os_error_t hosted_os_mempool_ext_clear(struct hosted_os_mempool_ext *mpe);
 ...
 */
bool hosted_os_mempool_is_sane(const struct hosted_os_mempool *mp);
 ...
 */
int hosted_os_memblock_from(const struct hosted_os_mempool *mp, const void *block_addr);
 ...
 */
void *hosted_os_memblock_get(struct hosted_os_mempool *mp);
 ...
 * @return os_error_t
 */
os_error_t hosted_os_memblock_put_from_cb(struct hosted_os_mempool *mp, void *block_addr);
 ...
 * @return os_error_t
 */
os_error_t hosted_os_memblock_put(struct hosted_os_mempool *mp, void *block_addr);
